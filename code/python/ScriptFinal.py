...
# ARCHIVO
# =========================
CSV_FILE = "Tesis RF Harvesting - MODELO COMPLETO.csv"
...
# En tu CSV, por cada distancia hay 2 filas y la segunda suele venir con Distancia = NaN.
# Esto reconstituye la distancia de esa segunda fila:
df[DIST_COL] = pd.to_numeric(df[DIST_COL], errors="coerce")
df[DIST_COL] = df[DIST_COL].ffill()  # <- clave para "dos filas por distancia"

# Detecta columnas de mediciones repetidas (vienen como "Unnamed: x")
unnamed_cols = [c for c in df.columns if c.startswith("Unnamed")]

# Lista total de columnas Vout (principal + repetidas)
vout_cols = [VOUT_MAIN_COL] + unnamed_cols

# Convierte a numérico (por si hay vacíos o texto)
for c in [DIST_COL] + vout_cols:
    df[c] = pd.to_numeric(df[c], errors="coerce")

# Pasa a formato largo: una fila por medición (incluye ambas filas por distancia)
long_df = df[[DIST_COL] + vout_cols].melt(
    id_vars=DIST_COL,
    value_vars=vout_cols,
    var_name="Medicion",
    value_name="Vout_V"
).dropna()
...
# =========================
summary = long_df.groupby(DIST_COL)["Vout_V"].agg(["count", "mean", "std"]).reset_index()
...
# =========================
# FIGURA: Distancia vs Vout con IC
# =========================
...
ax.scatter(
    ...
    long_df["Vout_V"].to_numpy(dtype=float),
    s=CFG["SCATTER_SIZE"],
...
ax.set_ylabel("Voltaje de salida (V)")

ax.grid(True, alpha=CFG["GRID_ALPHA"])
ax.legend()
ax.set_ylim(0, 0.3)   # <-- aquí cambias el rango (ej: 0 a 3 V)

# =========================
# Anotación movible (resumen)
# =========================
# Ejemplo: muestra resumen en la distancia máxima
...
# =========================
print("=== INFO ===")
print(f"Filas originales: {len(df)} (recuerda: 2 filas por distancia)")
print(f"Mediciones totales (long_df): {len(long_df)}")
